...
@Tag(name = "국내 운송 정보 관련", description = "해외 운송정보와 출발지, 도착지, 날짜에 관련한 기록 조회 기능을 제공하고 있습니다.")
public class TransferController {

    private final TransferService transferService;

    @PostMapping("/register")
    @Operation(summary = "국내 해외 운송 정보 등록", description = "운송 정보를 기록하는 API 입니다.")
    @ApiResponses(value = {
    ...
    @Operation(summary = "국내 해외 운송 도착 정보 조회", description = "도착지를 검색하면 해당 도착지로 기록된 운송 정보를 조회하는 API입니다.")
    @ApiResponses(value = {
    ...
    public ResponseEntity<?> arriveSearch(@Parameter(description = "도착지", example = "부여군", name = "arriveLocation") String arriveLocation,
    ...
    @Operation(summary = "국내 해외 운송 출발 정보 조회", description = "출발지를 검색하면 해당 출발지로 기록된 운송 정보를 조회하는 API입니다.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200",description = "조회 완료", content = @Content(schema = @Schema(
                    implementation = OverseasTransferArriveSearchRes.class
            ))),
    ...
    public ResponseEntity<?> departureSearch(@Parameter(description = "출발지", example = "광명시", name = "departureLocation") String departureLocation,
                                             @Parameter(description = "페이지", example = "0", name = "page")@RequestParam(required = false) Integer page,
                                             @Parameter(description = "사이즈", example = "2", name = "size")@RequestParam(required = false) Integer size) {
    ...
    @Operation(summary = "국내 물품 정보 조회 - 물품", description = "물품 정보로 검색하면 해당 물품의 모든 운송 정보를 조회하는 API입니다.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200",description = "조회 완료", content = @Content(schema = @Schema(
                    implementation = OverseasTransferArriveSearchRes.class
            ))),
    ...
    @Operation(summary = "국내 물품 정보 조회 - 운송장 번호", description = "운송장 번호로 검색하면 해당 물품의 모든 운송 정보를 조회하는 API입니다.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200",description = "조회 완료", content = @Content(schema = @Schema(
                    implementation = OverseasTransferArriveSearchRes.class
            ))),
    ...
    public ResponseEntity<?> wayBillSearch(@Parameter(description = "운송장 번호", example = "INV0001", name = "wayBill")@RequestParam String wayBill) {
    ...
    @Operation(summary = "국내 해당 날짜 이후 도착건 조회", description = "날짜를 검색하면 해당 날짜 포함 이후 날짜에 도착한 운송 정보를 조회하는 API입니다.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200",description = "조회 완료", content = @Content(schema = @Schema(
                    implementation = OverseasTransferArriveSearchRes.class
            ))),
    ...
    public ResponseEntity<?> departureDateSearch(@Parameter(description = "도착 날짜", example = "2023-11-17", name = "arriveDate") LocalDate arriveDate,
                                                 @Parameter(description = "페이지", example = "0", name = "page")@RequestParam(required = false) Integer page,
                                                 @Parameter(description = "사이즈", example = "2", name = "size")@RequestParam(required = false) Integer size) {
    ...
    @Operation(summary = "국내 해당 날짜 이후 출발건 조회", description = "날짜를 검색하면 해당 날짜 포함 이후 날짜에 출발한 운송 정보를 조회하는 API입니다.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200",description = "조회 완료", content = @Content(schema = @Schema(
                    implementation = OverseasTransferArriveSearchRes.class
            ))),
    ...
    public ResponseEntity<?> arrivalDateSearch(@Parameter(description = "출발 날짜", example = "2023-11-17", name = "departureDate") LocalDate departureDate,
                                               @Parameter(description = "페이지", example = "0", name = "page")@RequestParam(required = false) Integer page,
                                               @Parameter(description = "사이즈", example = "2", name = "size")@RequestParam(required = false) Integer size) {
